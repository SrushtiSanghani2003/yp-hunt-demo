...
const PortalUser = () => {
  ...
  const {
    ...
    isView,
    isUpdate
  } = getPermissionFlags(menuPermissions.portal_user);
  ...
  const handleViewUser = (id: number) => {
    if (!id) return;
    navigate(`/users/portaluser/view/${id}`);
  };
  ...
  const handleEditUser = (id: number) => {
    if (!id) return;
    navigate(`/users/portaluser/edit/${id}`); // 🔁 change route as per your app
  };
  ...
  useEffect(() => {
    if (userData?.data?.users) {
      setPortalUsers(userData.data.users);
    }
  }, [userData]);
  ...
  return (
    <div>
      ...
      <div className="container py-3 flex items-end justify-between mt-5">
        ...
        <ExportDropdown model="user" isDisable={!isExport} />
      </div>
        ...
        <table className="min-w-full table-fixed border-separate border-spacing-y-2">
          ...
          <tbody>
            {isFetching ? (
              ...
              (portalUsers as any[])?.map((users, index) => {
      ...
      {portalUsers.length !== 0 && totalPages > 1 && (
        <Pagination
          currentPage={page}
