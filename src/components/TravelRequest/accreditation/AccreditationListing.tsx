...
import {
  useAccreditationlist,
  useDownloadAccreditationTemplate,
  useExportCsvAccreditation,
...
export default function AccreditationListing() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { isScrolled } = useScroll();
  const module = "accreditation";
  ...
  const { isUpdate, isView } = getPermissionFlags(
    menuPermissions.accreditation,
  );
  const { mutate: downloadTemplate, isPending } =
    useDownloadAccreditationTemplate();
  const { mutate: exportCSV, isPending: exportCsvPanding } =
    useExportCsvAccreditation(status);
  const [allAccreditation, setAllAccreditation] = useState([]);
  ...
  const { data: allAccreditationData, isFetching } = useAccreditationlist(
    page,
    debouncedSearch,
    status,
    fromDate,
  ...
  const totalPages = useMemo(() => {
    return allAccreditationData?.totalPages || 1;
  }, [allAccreditationData?.totalPages]);

  const handleAccreditation = (id: number) => {
    if (!id) return;
    navigate(`/travelrequest/accreditation/edit/${id}`);
  };
  useEffect(() => {
    if (debouncedSearch) {
      setPage(1);
  ...
  useEffect(() => {
    if (allAccreditationData?.accreditation) {
      setAllAccreditation(allAccreditationData.accreditation || []);
    }
  }, [allAccreditationData]);
  return (
    <div>
      <div
      ...
      <div className="container">
        <div className="flex justify-end items-center mt-3 gap-3">
          ...
          <Button
            ...
            className="!px-4 md:!py-2.5"
            onClick={() =>
              navigate(paths.travelrequest.accreditation.uploadCsv.path)
            }
            icon={arrowupcutout}
          />
          <Button
          ...
          <table className="min-w-full table-fixed border-separate border-spacing-y-2 ">
            ...
            <tbody>
              {isFetching ? (
                ...
                (allAccreditation as any[])?.map((accreditation, index) => {
                  const isConfirmed =
                    accreditation?.status?.toLowerCase() === "confirmed";
                  return (
                              ...
                              <tr>
                                ...
                                <PhotoProvider maskOpacity={0.6}>
                                  <td className="text-base cursor-pointer">
                                    <PhotoView
                                      src={concatImgURL(
                                        accreditation.requested_for_image,
                                      )}
                                    >
                                      {accreditation.requested_for_image ? (
                                        <img
                                ...
                                <td className="text-base text-left">
                                  <p className="w-36 line-clamp-2">
                                    {accreditation.requested_for_name
                                      ? accreditation.requested_for_name
                                      : "-"}
                                  </p>
                                </td>
                                <td className="text-base text-left">
                                  <p className="w-36 line-clamp-2">
                                    {accreditation.requested_by_name
                                      ? accreditation.requested_by_name
                                      : "-"}
                                  </p>
                                </td>
                                ...
                                <td className="text-base text-left">
                                  {formatDate(accreditation.requested_on)}
                                </td>
      ...
      {showSidebar && (
        <AccreditationView
          ...
          accreditationId={selectedAccreditationId}
        />
      )}
      {allAccreditation.length !== 0 && totalPages > 1 && (
        <Pagination
          currentPage={page}
