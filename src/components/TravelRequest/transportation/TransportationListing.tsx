...
import {
  useDownloadTransportationTemplate,
  useExportCsv,
  useTransportationlist,
...
export default function TransportationListing() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { isScrolled } = useScroll();
  const module = "transportation";
  ...
  const { isUpdate, isView } = getPermissionFlags(
    menuPermissions.transportation,
  );
  const [allTransportation, setAllTransportation] = useState([]);
  ...
  const { data: allTransportationData, isFetching } = useTransportationlist(
    page,
    debouncedSearch,
    status,
    fromDate,
  ...
  const totalPages = useMemo(() => {
    return allTransportationData?.totalPages || 1;
  }, [allTransportationData?.totalPages]);

  const handleTransportation = (id: number) => {
    if (!id) return;
    navigate(`/travelrequest/transportation/edit/${id}`);
  };
  useEffect(() => {
    if (debouncedSearch) {
      setPage(1);
  ...
  useEffect(() => {
    if (allTransportationData?.transportation) {
      setAllTransportation(allTransportationData.transportation || []);
    }
  ...
  return (
    <div>
        ...
        <div>
          <div className="flex justify-end items-center mt-3 gap-3">
            ...
            <Button
              ...
              className="!px-4 md:!py-2.5"
              onClick={() =>
                navigate(paths.travelrequest.transportation.uploadCsv.path)
              }
              icon={arrowupcutout}
            />
          ...
          <table className="min-w-full table-fixed border-separate border-spacing-y-2 ">
            ...
            <tbody>
              {isFetching ? (
                ...
                (allTransportation as any[])?.map((transportation, index) => {
                  const isConfirmed =
                    transportation?.status?.toLowerCase() === "confirmed";

                  return (
                    <tr key={transportation?.id} className="">
                      <td colSpan={columns.length}>
                              ...
                              <tr>
                                ...
                                <td className="text-base text-left">
                                  <p className="w-36 line-clamp-2">
                                    {transportation.requested_for_name
                                      ? transportation.requested_for_name
                                      : "-"}
                                  </p>
                                </td>
                                <td className="text-base text-left">
                                  <p className="w-36 line-clamp-2">
                                    {transportation.requested_by_name
                                      ? transportation.requested_by_name
                                      : "-"}
                                  </p>
                                </td>
                                <td className="text-base text-left">
                                  {formatDate(transportation.requested_on)}
                                </td>
                                ...
                                <td className="text-base text-left">
                                  <p className="w-28 line-clamp-2">
                                    {transportation.tournament_name
                                      ? transportation.tournament_name
                                      : "-"}
                                  </p>
                                </td>
      ...
      {allTransportation.length !== 0 && totalPages > 1 && (
        <Pagination
          currentPage={page}
