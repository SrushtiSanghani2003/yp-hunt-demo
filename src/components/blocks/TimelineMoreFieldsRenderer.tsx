...
const TimelineMoreFieldsRenderer = ({
  currentBlock,
  timeline,
  moreFields,
  setMoreFields,
  ...
  const handleRemoveMoreField = (index: number, keyPath: string) => {
    ...
    const timeline = newBlock.content.timelines[index];
    const fieldData = timeline.more?.[fieldKey];
  ...
  useEffect(() => {
    timeline.more?.cta?.forEach((ctaItem: any, idx: number) => {
      ...
      <div className="md:mt-5 mt-2 relative">
        ...
        {moreFields.includes("description") && (
          <div className="relative mb-3">
            ...
            <FroalaEditor
              model={timeline.more?.description}
              onModelChange={(e) =>
            ...
            {/* <textarea
              ...
              rows={2}
              value={timeline.more?.description || ""}
              onChange={(e) =>
        ...
        {moreFields.includes("bullet") &&
          timeline.more?.bullet?.map((bulletItem: any, idx: number) => (
        ...
        {moreFields.includes("cta") &&
          timeline.more?.cta?.map((ctaItem: any, idx: number) => (
            <div
              key={ctaItem.id}
        ...
        {moreFields.includes("sponsor") &&
          timeline.more?.sponsor?.map((sponsorItem: any, idx: number) => (
        ...
        {moreFields.includes("social") && (
          <div className="mb-4 relative">
            ...
            {/* Fields */}
            {timeline.more?.social?.map((socialItem: any, idx: number) => {
              // Get all selected platforms except current one
              const selectedPlatforms = timeline.more.social
...
export default TimelineMoreFieldsRenderer;
