...
const Articles4V = ({ data, title }: Props) => {
  return (
    <div className="my-4">
      ...
      <Swiper spaceBetween={16} slidesPerView={1} className="!px-1">
        {chunkArray(data, 4).map((group, index) => (
            ...
            <div className="flex gap-4 h-60">
              {group.map((item: any) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="w-[25%] border-primary border-0.5 rounded-2xl flex flex-col overflow-hidden h-60"
                  >
