...
const AdvertisementMoreFieldsRenderer = ({
  currentBlock,
  ad,
  moreFields,
  setMoreFields,
  ...
  const handleMoreFieldChange = (
    ...
    const updatedAds = currentBlock.content.ads.map((ad: any, i: number) => {
      ...
      let updatedMore = structuredClone(ad.more || {});

      // 1️⃣ Apply the main change
      updatedMore = setNestedValue(updatedMore, keyPath, value);

      // 2️⃣ If target_is_blank changes, reset related button_link
      if (keyPath.includes(".target_is_blank")) {
        const buttonLinkPath = keyPath.replace(
          "target_is_blank",
          "button_link"
        );
        updatedMore = setNestedValue(updatedMore, buttonLinkPath, "");
      }
  ...
  const handleRemoveMoreField = (index: number, keyPath: string) => {
    const fieldKey = keyPath.split(".")[0]; // e.g., "title" from "title.0.value"
    ...
    // Check if the field still exists in the updated ad
    const ad = newBlock.content.ads[index];
    const fieldData = ad.more?.[fieldKey];
  ...
  useEffect(() => {
    ad.more?.cta?.forEach((ctaItem: any, idx: number) => {
      // skip if already selected
      if (selectedPages[idx]) return;
      ...
      if (ctaItem.module) {
        ...
        const initialItem =
          dataForModule?.find((item: any) => item.slug === ctaItem.slug) ||
          null;

        if (initialItem) {
          // ✅ update both selectedOptions and selectedPages
          setSelectedOptions((prev) => ({
            ...prev,
            [idx]: { value: initialItem.slug, label: initialItem.title },
          }));

          setSelectedPages((prev) => ({
            ...prev,
            [idx]: { value: initialItem.id, label: initialItem.title },
          }));

          // update slug in cta if missing
          if (!ctaItem.slug) {
  ...
  }, [pageModule.data, newsModule.data, ad.more]);
      ...
      <div className="md:mt-5 mt-2 relative">
        ...
        {moreFields.includes("description") && (
          <div className="relative mb-3">
            ...
            <FroalaEditor
              model={ad.more?.description || ""}
              onModelChange={(e) =>
                handleMoreFieldChange(index, "description", e)
              }
              isDark={currentBlock?.content?.is_dark}
              onThemeChange={(value: any) =>
                handleMoreFieldChange(index, "is_dark", value)
              }
            />
            {/* <textarea
        ...
        {moreFields.includes("bullet") &&
          ad.more?.bullet?.map((bulletItem: any, idx: number) => (
        ...
        {/* {moreFields.includes("cta") &&
          ad.more?.cta?.map((ctaItem: any, idx: number) => (
            <div
              key={ctaItem.id}
              className="grid grid-cols-3 items-end gap-5 relative mb-3"
            >
              <Input
                label="Button Text"
                className="m-0"
              ...
              <Input
                ...
                note="Note: Include http:// or https://"
                className="m-0"
        ...
        {moreFields.includes("cta") &&
          ad.more?.cta?.map((ctaItem: any, idx: number) => {
            return (
              <div
                key={ctaItem.id}
                className="grid grid-cols-12 items-end gap-5 relative mb-3"
              >
                <Input
                  label="Button Text"
              ...
              /> */}
                ...
                {ctaItem?.target_is_blank ? (
                  <Input
                    label="Button Link URL"
                    className={`m-0 ${
                      ctaItem?.target_is_blank ? "col-span-6" : "col-span-4"
                    }`}
                    value={ctaItem?.button_link ?? ""}
                    onChange={(e) =>
                      handleMoreFieldChange(
                  ...
                  <>
                    ...
                    <div className="relative col-span-2">
                      ...
                      options={
                        ctaItem.module === "page"
                          ? pageModule.data?.map((item: any) => ({
                              value: item.slug,
                              label: item.title,
                            }))
                          : newsModule.data?.map((item: any) => ({
                              value: item.slug,
                              label: item.title,
                            }))
                      }
                      // selectedModule={selectedModule}
                      selected={selectedOptions[idx] || null}
                      onSelect={(item: any) => {
                        setSelectedOptions((prev) => ({
                          ...prev,
                          [idx]: item,
                      ...
                      <TreeSelect
                        ...
                        onNodeSelect={(e) => {
                          if (
                            ...
                            fetchFn().then(() => {
                              setTimeout(() => {
                                treeSelectRefs.current[idx]?.show();
                              }, 150);
                            });
                            return;
                          }
        ...
        {moreFields.includes("sponsor") &&
          ad.more?.sponsor?.map((sponsorItem: any, idx: number) => (
...
export default AdvertisementMoreFieldsRenderer;
