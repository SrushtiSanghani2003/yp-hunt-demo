...
const columns = [
  ...
  { title: "Type", minWidth: "min-w-32 w-32" },
...
const Membership = () => {
  ...
  const getPages = async ({
    ...
    return await api.get("/membership", { params });
  };

  const { data: pagesData, isFetching } = useQuery({
    queryKey: ["membership", page, debouncedSearch],
    queryFn: getPages,
  ...
  const deletePage = async (id: string) => {
    return await api.delete(`/membership/${id}`);
  };
  ...
  const statusChange = async (statusId: string) => {
    ...
    return await api.post(`/membership/${statusId}/change-status`, {
      status: isActive,
    });
  };
  ...
  const handleCreate = () => {
    dispatch(resetPage());
    navigate(paths.membership.create.getHref());
  };

  const updateStatus = async (payload: any) => {
    await api.patch(`/membership/${selectedPage?.id}/change-status`, payload);
  };
  ...
  const handleOrderClick = async (
    ...
    if (isConfirmed && new_order && parseInt(new_order) !== current_order) {
      ...
      try {
        await api.patch(`/common/change-order/Membership/${id}`, payload);
  ...
  useEffect(() => {
    if (pagesData?.data?.memberships) {
      setAllPages(pagesData.data.memberships);
    }
  }, [pagesData]);
                              ...
                              {visibleColumns.includes("Title") && (
                                <td className="text-base text-left">
                                  ...
                                  >
                                    ...
                                    >
                                      {page.translation.title.length > 80
                                        ? `${page.translation.title.slice(
                                            0,
                                            80
                                          )}...`
