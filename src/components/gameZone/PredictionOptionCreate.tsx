...
import "./normalizer/normalizeQuizQuestion";
...
import ClosedAtDateModal from "./ClosedDateModal";
import {
  denormalizePredictionQuestionsFromAPI,
  normalizePredictionQuestionsForAPI,
} from "./normalizer/normalizePredictionQuestion";
import { concatImgURL } from "../../config/function";

export default function PredictionOptionCreate() {
  ...
  const getQuestionsByQuizId = async (predictionId: string) => {
    const res = await api.get(`/prediction/${predictionId}/questions/`);
    return res.data;
  };

  const { data: questionsDataByQuizId } = useQuery({
    queryKey: ["questionsDataByQuizId"],
    queryFn: () => getQuestionsByQuizId(state.predictionId),
    enabled: !!state.predictionId,
  });

  useEffect(() => {
    if (questionsDataByQuizId?.questions.length > 0) {
      
      const gotQuestions: any = denormalizePredictionQuestionsFromAPI(
  ...
  const handleQuestionChange = (
    index: number,
    field: "type" | "closed_at" | "question_mode" | "question_text" | "order",
  ...
  const addQuestion = async () => {
    const normalizedQuestions = normalizePredictionQuestionsForAPI(
      questions,
      false
    );

    const res = await api.post(
      `/prediction/${state.predictionId}/questions`,
      normalizedQuestions
    );
    return res.data;
  };
  ...
  const updatedQuestion = async () => {
    ...
    const res = await api.put(
      `/prediction/questions/${state.predictionId}`,
      normalizedQuestions
    );
    return res.data;
  };
  ...
  return (
    <div className="mt-10">
        ...
        <div className="lg:col-span-12 md:col-span-12 col-span-12 mb-5">
          ...
          {questions.map((question, questionIndex) => (
            ...
            >
              ...
              <div className="mt-3">
                <Input
                  label={`Predictor ${questionIndex + 1}`}
                  className="mb-2"
              ...
              <div className="md:mt-5 mt-3">
                <div className="grid md:grid-cols-2 grid-cols-1 items-center md:gap-5">
                  <div>
                    <label className="block w-full font-medium md:text-base text-sm mb-0">
                      Choice
                    </label>
                    <div
                      ref={(el) => {
                  ...
                  <div className="">
                    ...
                    <input
                      ...
                      value={
                        question.closed_at
                          ? dayjs(question.closed_at).format("YYYY-MM-DDTHH:mm")
                          : ""
                      }
                      onClick={() => {
                      ...
                      className={`w-full p-2 md:text-base text-sm border-0.5 border-primary rounded-xl`}
                    />
                  </div>
                </div>
      ...
      {dateModal && activeQuestionIndex != null && (
        <ClosedAtDateModal
          show={dateModal}
          onClose={() => {
            setDateModal(false);
          ...
          //   utcDate={utcDate}
          utcDate={questions[activeQuestionIndex].closed_at}
          //   setUtcDate={(date: any) => setUtcDate(date)}
          setUtcDate={(selectedDate: string | null) => {
            handleQuestionChange(
              activeQuestionIndex,
              "closed_at",
              selectedDate
